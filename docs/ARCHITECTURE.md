...
This is a small MCP server. Roughly 600 lines of TypeScript across three
...
- `package.json` carries the current alpha version (`0.1.1-alpha.0`
  at the time of writing).
- Git tags mark notable snapshots (`v0.1.0-alpha.0`,
  `v0.1.1-alpha.0`) so reviewers can pin to a specific point in time.
- Backwards-compatibility commitments are deliberately not made at
  this stage; the project is in working-draft status.
...
- [`LICENSE`](../LICENSE) — current draft licensing posture
- `EXAMPLES.md` (forthcoming) — request / response samples per tool
- `ROADMAP.md` (forthcoming) — Layer 1 / 2 / 3 positioning
